...
export function highlightKeys(
  ...
  // Names and keywords come from cloud APIs; escape them so the highlight
  // markup cannot smuggle HTML into v-html (XSS).
  ...
  keys.forEach(n => {
    if (!n) return;
    const escapedKeyword = escapeHtml(n);
    list.forEach(m => {
      m.highlightName = escapeHtml(m.name).replace(
        escapedKeyword,
        `<span class="highlight-text">${escapedKeyword}</span>`
      );
    });
  });
  if (typeof data === 'string') {
