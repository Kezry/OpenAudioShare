...
+ 自动音量: 开启后自动把不同电脑的采集响度归一到相同电平，解决不同 PC 间音量大小不一致的问题。
+ 驱动适配: 自动识别 Realtek 公版、杜比、联想智音（Waves MaxxAudio）等声卡驱动并选择适配参数，也可手动切换。
...
不同电脑的声卡驱动差异较大，若播放出现断续、无声或爆音，可在 Windows 端 “采集模式” 下拉框中手动切换：

+ 默认（轮询 100ms）：绝大多数设备的常规模式。
+ 事件驱动（100ms）：部分驱动在轮询模式下出声异常，事件驱动更稳定。
+ 低延迟（事件 20ms）：事件驱动的低延迟版本，对网络与机器性能要求略高。
+ 兼容（轮询 200ms）：配置较低或网络抖动较大的环境使用。

切换采集模式不会断开播放设备，音频会在短暂间隙后自动继续。音频采集异常时会自动尝试恢复；设备被拔出后重新插入（或驱动重置恢复）也会自动恢复采集，并以系统通知提示异常原因。

### 自动音量

不同电脑的混音电平不同（混音器里各程序的音量、驱动音频增强、片源响度都会影响），即使系统音量调到最大，流出的声音也可能偏大或偏小。开启 Windows 端 “自动音量” 开关后，软件会实时测量采集电平并把增益自动收敛到统一响度（约数秒完成适应），不同电脑听到的音量就基本一致了。增益滑条仍可在自动调节的基础上手动微调。

### 驱动适配

不同品牌电脑的声卡驱动（联想智音 / Waves MaxxAudio、杜比、Realtek 公版等）内置的音频处理不同，会直接影响环回采集到的信号。软件默认 “自动检测”：根据声卡驱动、系统服务与已安装程序识别机器上的 DSP 方案——

+ 杜比类驱动自带响度归一与压限，自动音量会使用更保守的参数（低增益上限、慢收敛），避免二次动态处理造成“呼吸感”；
+ 联想智音 / MaxxAudio 类驱动同样使用保守响度参数，并且在您未手动选择采集模式时默认采用事件驱动采集（这类驱动轮询模式下更易断续）；
+ Realtek 公版驱动信号偏安静，自动音量目标电平略高；
+ 未识别出 DSP 时使用通用参数。

检测不准或需要强制指定时，可在 “适配” 下拉框手动切换（自动检测 / 通用 / Realtek / 杜比 / 智音），立即生效。
